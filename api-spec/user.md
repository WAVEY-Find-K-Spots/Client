...
| nickname / nationality / language 프로필 확장 | - | 미착수 (필드 자체가 아직 없음) |
...
- **현재 `UserResponse`에는 `nickname`, `nationality`, `language` 필드가 없습니다.** 프론트에서 필요하면 백엔드 작업이 선행되어야 합니다.
...
- `nickname`, `nationality`, `language` 필드 추가 (User 엔티티/응답 DTO 확장 필요, enum 설계 포함)
