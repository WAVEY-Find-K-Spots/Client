...
| 그 외 카테고리 (게시물, 스팟 이미지 등) | 미구현 — `UploadCategory` enum에 추가 필요 |
...
```ts
{
  category: "PROFILE"     // @NotNull, UploadCategory enum (현재 PROFILE만 존재)
  contentType: string     // @NotBlank, 예: "image/png"
}
```

허용되는 `contentType` (카테고리별, 현재 `PROFILE`만):
- `image/jpeg` → `.jpg`
- `image/png` → `.png`
- `image/webp` → `.webp`
...
- `UploadCategory`에 `PROFILE` 외 카테고리 추가 (게시물 이미지, 스팟 이미지 등)
